...
function TransactionFormScreen({ navigation }: { navigation: any }) {
    const [loading, setIsLoading] = useState(false);
    const { transactionData, setTransactionData, resetTransaction, upiParams, setUpiParams } = useTransaction();
    const descriptionRef: RefObject<TextInput> = useRef<TextInput>(null);
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        if (group && group.members) {
            ...
            setTransactionData((prev: any) => ({
                ...prev,
                splitAmong: group.members.map((user: { _id: any; }, index: number) => ({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const newTransaction = {
                ...
                splitAmong: transactionData.splitAmong.map((user: { amount: any; user: { _id: any; id: any; }; }) => ({
                    amount: user.amount,
            ...
            const newActivity = {
                relatedId: {
                    ...
                    splitAmong: transactionData.splitAmong.map((user: { amount: any; user: { _id: any; id: any; }; }) => ({
                        amount: user.amount,
                        user: transactionData.group.members.find((member: { _id: any; }) => member._id === (user.user._id || user.user.id)),
                    })),
                },
    ...
    return loading ? (
        ...
        <ScrollView style={styles.container} alwaysBounceVertical={false}>
            <AmountInput amount={transactionData.amount} handleInputChange={(text: any) => handleInputChange('amount', text)} isTextInput />
            ...
            >
                <Button
                        ...
                        title="Submit" disabled={undefined} loading={undefined}                />
            </View>
        </ScrollView>
    );
}
